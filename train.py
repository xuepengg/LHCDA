...
parser.add_argument('--data_src', type=str, default='acmv9', help='source dataset name')
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, (batch_s, batch_t) in enumerate(
            ...
            feat_t, pred_label_t, adj_t = (torch.FloatTensor(feat_t.toarray()),
                                           ...
                                           torch.FloatTensor(adj_t.toarray())
                                           )
            ...
            loss_nl = 0
            # positive learning
            pred_logit_node_t_softmax = F.softmax(pred_logit_t, dim=1)
            positive_idx = np.array(torch.where(pred_logit_node_t_softmax > 0.8)[0].cpu())
            for i in range(0, 5):
                nl_idx = np.array(torch.where(pred_logit_node_t_softmax[:, i] < 0.2)[0].cpu())
                # #negative learning
                if nl_idx.size > 0:
                    nl_logits = pred_logit_t[nl_idx]
            ...
            comlabel_inter_st = torch.mm(label_s.float(), pred_label_t.t())  ##check whether two nodes in s and t have same label
            comlabel_inter_ts = torch.mm(pred_label_t, label_s.float().t())  ##check whether two nodes in s and t have same label
            ...
            centroid_s = calculate_centroid_2(label_s.float(), emb_s)
            centroid_t = calculate_centroid_2(pred_label_t, emb_t)
            cross_view_st_nc = inter_view_nei_loss(emb_s, centroid_t, args.tau, label_s)
            cross_view_ts_nc = inter_view_nei_loss(emb_t, centroid_s, args.tau, pred_label_t)
            cross_network_constrastive_loss_NC = (cross_view_st_nc + cross_view_ts_nc) * 0.5
            cross_network_constrastive_loss_NC = cross_network_constrastive_loss_NC.mean()

            total_loss = domain_loss + clf_loss +args.inter_view_gcl_wei * cross_network_constrastive_loss + args.inter_view_gcl_wei_nc * cross_network_constrastive_loss_NC + loss_nl
